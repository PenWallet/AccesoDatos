...
AS
	BEGIN
		...
		-- TABLA TEMPORAL EN LA QUE ALMACENAREMOS LA FILA CON LA QUE TRABAJAMOS, PARA NO HACER SELECT PESADOS
		CREATE TABLE #Temp2
		   (ID INT,
			N1 SMALLINT,
			N2 SMALLINT,
		...
		INSERT INTO #Temp
			SELECT APS.ID, APS.N1, APS.N2, APS.N3, APS.N4, APS.N5, APS.N6
		...
		WHILE @ContTablas > 0
			BEGIN
				INSERT INTO #Temp2
					SELECT TOP 1 * FROM #Temp ORDER BY ID
				SET @ContAciertos = 0
				SET @IDTemp = (SELECT ID FROM #Temp2)
				SET @N1Temp = (SELECT N1 FROM #Temp2)
				SET @N2Temp = (SELECT N2 FROM #Temp2)
				SET @N3Temp = (SELECT N3 FROM #Temp2)
				SET @N4Temp = (SELECT N4 FROM #Temp2)
				SET @N5Temp = (SELECT N5 FROM #Temp2)
				SET @N6Temp = (SELECT N6 FROM #Temp2)
...
--SELECT * FROM SORTEO
--EXEC dbo.GrabaMuchasSencillas 2, 1000
--EXEC dbo.AsignarPremiosCat5 2
--SELECT * FROM APUESTA_SIMPLE WHERE GANADO IS NOT NULL AND ORDER BY ID DESC
--SELECT * FROM APUESTA